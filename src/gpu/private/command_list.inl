...
namespace GPU
{
	...
	INLINE CommandUpdateRTV* CommandList::UpdateRTV(Handle frameBinding, i32 firstRTV, i32 numRTVs)
	{
		DBG_ASSERT(handleAllocator_.IsValid(frameBinding));
		DBG_ASSERT(frameBinding.GetType() == ResourceType::FRAME_BINDING_SET);
		DBG_ASSERT(firstRTV >= 0);
		DBG_ASSERT(numRTVs > 0);

		auto* command = Alloc<CommandUpdateRTV>();
		command->frameBinding_ = frameBinding;
		command->firstRTV_ = (i16)firstRTV;
		command->numRTVs_ = (i16)numRTVs;
		command->rtvs_ = Alloc<BindingRTV>(numRTVs);
		commands_.push_back(command);
		return command;
	}

	INLINE CommandUpdateDSV* CommandList::UpdateDSV(Handle frameBinding, const BindingDSV& dsv)
	{
		DBG_ASSERT(handleAllocator_.IsValid(frameBinding));
		DBG_ASSERT(frameBinding.GetType() == ResourceType::FRAME_BINDING_SET);

		auto* command = Alloc<CommandUpdateDSV>();
		command->dsv_ = dsv;
		commands_.push_back(command);
		return command;
	}

	INLINE CommandUpdateSRV* CommandList::UpdateSRV(Handle pipelineBinding, i32 firstSRV, i32 numSRVs)
	{
		DBG_ASSERT(handleAllocator_.IsValid(pipelineBinding));
		DBG_ASSERT(pipelineBinding.GetType() == ResourceType::PIPELINE_BINDING_SET);
		DBG_ASSERT(firstSRV >= 0);
		DBG_ASSERT(numSRVs > 0);

		auto* command = Alloc<CommandUpdateSRV>();
		command->pipelineBinding_ = pipelineBinding;
		command->firstSRV_ = (i16)firstSRV;
		command->numSRVs_ = (i16)numSRVs;
		command->srvs_ = Alloc<BindingSRV>(numSRVs);
		commands_.push_back(command);
		return command;
	}

	INLINE CommandUpdateUAV* CommandList::UpdateUAV(Handle pipelineBinding, i32 firstUAV, i32 numUAVs)
	{
		DBG_ASSERT(handleAllocator_.IsValid(pipelineBinding));
		DBG_ASSERT(pipelineBinding.GetType() == ResourceType::PIPELINE_BINDING_SET);
		DBG_ASSERT(firstUAV >= 0);
		DBG_ASSERT(numUAVs > 0);

		auto* command = Alloc<CommandUpdateUAV>();
		command->pipelineBinding_ = pipelineBinding;
		command->firstUAV_ = (i16)firstUAV;
		command->numUAVs_ = (i16)numUAVs;
		command->uavs_ = Alloc<BindingUAV>(numUAVs);
		commands_.push_back(command);
		return command;
	}

	INLINE CommandUpdateCBV* CommandList::UpdateCBV(Handle pipelineBinding, i32 firstCBV, i32 numCBVs)
	{
		DBG_ASSERT(handleAllocator_.IsValid(pipelineBinding));
		DBG_ASSERT(pipelineBinding.GetType() == ResourceType::PIPELINE_BINDING_SET);
		DBG_ASSERT(firstCBV >= 0);
		DBG_ASSERT(numCBVs > 0);

		auto* command = Alloc<CommandUpdateCBV>();
		command->pipelineBinding_ = pipelineBinding;
		command->firstCBV_ = (i16)firstCBV;
		command->numCBVs_ = (i16)numCBVs;
		command->cbvs_ = Alloc<BindingBuffer>(numCBVs);
		commands_.push_back(command);
		return command;
	}
