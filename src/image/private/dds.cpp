...
#define MAKEFOURCC(A, B, C, D) ((u32(D) << 24) | (u32(C) << 16) | (u32(B) << 8) | (u32(A)))
		...
		ImageFormat GetFormat(DXGI_FORMAT format)
		{
			switch(format)
			{
			...
			case DXGI_FORMAT_BC6H_UF16:
				return ImageFormat::BC6H_UF16;
			case DXGI_FORMAT_BC6H_SF16:
				return ImageFormat::BC6H_SF16;
		...
		ImageFormat GetResourceFormat(D3DFORMAT Format)
		{
			switch(Format)
			{
			case D3DFMT_DXT1:
		...
		Image LoadImage(Core::File& imageFile, ErrorHandlerFn errorHandlerFn)
		{
			u32 magic = 0;
			...
			if(magic != HEADER_MAGIC_ID)
			{
				if(errorHandlerFn)
				{
					Core::Array<char, 1024> buffer;
					sprintf_s(buffer.data(), buffer.size(), "Unable to load \"%s\", header magic ID mismatch.",
					    imageFile.GetPath());
				}
				return Image();
			}
			...
			{
				type = ImageType::TEX3D;
			}
			if(Core::ContainsAllFlags(ddsHeader.dwCaps2, FlagsCube))
			{
				type = ImageType::TEXCUBE;
			}

			ImageFormat format = DDS::GetResourceFormat(DDS::D3DFORMAT(ddsHeader.ddspf.dwFourCC));
